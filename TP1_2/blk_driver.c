...
int rb_ioctl (struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case IO_CRYPT: // IOCTL CRYPT IS CALLED
            printk (KERN_ALERT "IOCTL CRYPT \n");
            char psswd_tmp [25];	// Passwd max 24 characters

			// Register the passwd
            copy_from_user (psswd_tmp, (char *) arg, 25);
            if (strcmp (psswd, "") == 0) // if passwd == "" crypting disk
                strcpy (psswd, psswd_tmp);
            else {
                if (strcmp (psswd, psswd_tmp) != 0) {
                    printk (KERN_ALERT "WRONG PASSWD \n");
                    return 2;
                }
                else	// decrypting disk
                    strcpy (psswd, "");
            }
            psswd_size = strlen(psswd_tmp);

			// crypting or decrypting the disk
            for (i = 0; i < rb_dev.size*LENGHT_SECTOR; i++)
                rb_dev.data[i] ^= psswd_tmp[i%psswd_size];

            return 0;
    }

    printk(KERN_ALERT "UNKNOWN IOCTL CALL: %x\n", cmd);
    printk(KERN_ALERT "IOCTL: type=%x\tnumber=%x\tdirection=%x\tsize=%x\n", _IOC_TYPE(cmd), _IOC_NR(cmd), _IOC_DIR(cmd), _IOC_SIZE(cmd));
    return 1;
}
